...
In this section ways of achieving these quality requirements are discussed. This is done through architectural tactics and strategies. An architectural tactic is a design decision for realizing quality goals at the architectural level. Tactics influence the control of a quality attribute. On the other hand an architectural strategy is a collection of tactics.
...
\subsubsection{Performance}
With performance the goals that need to be achieved were described as having speedy data transfer that can only be slowed down by the speed of the network, concurrency so the system can handle all hundred potential users at once and the system should be efficient and not lag in information delivery. 

\begin{figure}[H]
    \centering
    \caption{Performance Tactics}
    \includegraphics[width=1\textwidth]{arch-tactics/performance.png}
\end{figure}

In order to address performance, our strategies focus on the management of resources, management of the demand on resources as well as dealing with resource arbitration. Our strategies achieve this through concurrency and scheduling tactics, mainly but also by reducing and spreading the load. The major resource in our system will be the databases which stores information about users and publications, through the tactics described we will able to control the use of these resources as well as avoid conflicting accesses. This is crucial because this could firstly cause anomalies but more importantly slow down the system and give users a negative perception of the system if they have to wait to access resources. 
...
\begin{figure}[H]
    \centering
    \caption{Reliability Tactics}
    \includegraphics[width=1\textwidth]{arch-tactics/reliability.png}
\end{figure}

In order to maintain reliability of the system, the strategies we apply are there to prevent system failures, and in the rare case that they do occur ensure they are detected and make appropriate preparations to recover and return to a working state of the system. In order to prevent faults in the system tactics such as error checking and process monitoring are applied, this way we can avoid human errors and ensure all operations are running smoothly. To detect faults we make use of logging and resource monitoring, this means making note of any changes or actions to the database and ensuring that it is always functional and that no potential faults exist in the databases. To recover from faults one should always backup the system when it is a working state, and use passive redundancy to inform system components when they need to make state updates. Thereafter once a fault occurs one can always rollback to the most recent working state.

\subsubsection{Scalability}
For scalability the goals that need to be achieved include the system being expandable to other faculties at the University, with there being a total of 11 faculties that means the system should thus be able to handle 1100 users concurrently without hassle and still meet the same performance standards as if there were only a 100 users. 

\begin{figure}[H]
    \centering
    \caption{Scalability Tactics}
    \includegraphics[width=1\textwidth]{arch-tactics/scalability.png}
\end{figure}

The strategies we use to address the scalability of our system to achieve the goal of it being expandable includes the management of resources, this is necessary so that the resources can be managed to deal with the greater demand once the system is expanded to more faculties. In addition the resources will need to be scaled up so that, it can be applied in the different contexts of various faculties hence tactics such as using abstract entities and the support of database field additions. 

\subsubsection{Usability}
Goals that need to be achieved for the usability quality requirement is that the user should be able to perform system operations without hassle, the user interface should not be cluttered and information access by user’s needs to be simple.

\begin{figure}[H]
    \centering
    \caption{Usability Tactics}
    \includegraphics[width=1\textwidth]{arch-tactics/usability.png}
\end{figure}

Our chosen usability tactics focus on making the system easier to use for user's. The strategy of having a user interface is important and includes tactics such as using Human Computer Interaction(HCI) techniques as well as having a graphical user interface to make the user feel at ease with the system. In addition information coherence as a tactic is important because it achieves the goal of information access and avoiding clutter and redundancy of information. Also another strategy is having the system able to interact with th user through run-time with tactics like user commands and giving the user feedback on their operations, this makes the sytem more usable because it gives the user a semblance of control and keeps them informed.

\subsubsection{Auditability}
For auditability, goals that need to be achieved is the tracking of all information associated with the system including errors, changes made, when such occurred and who performed what actions thus creating an audit trail for the system.

\begin{figure}[H]
    \centering
    \caption{Auditability Tactics}
    \includegraphics[width=1\textwidth]{arch-tactics/auditability.png}
\end{figure}

The strategy to deal with auditability is logging. We will make use of log files to implement tactics like tracking all actions/changes, track the users activity and  track errors as well making use of labels on logs that include time stamps, location of changes or errors as well as who made what changes and what caused errors. This all contributes to enabling system auditability because everything is tracked and stored separate from the system databases.

\subsubsection{Security}
With Security the goals that need to be achieved includes a log in system of which the user details will be verified and the prevention of unauthorized access.

\begin{figure}[H]
    \centering
    \caption{Security Tactics}
    \includegraphics[width=1\textwidth]{arch-tactics/security.png}
\end{figure}

The strategies for addressing the security quality requirement includes resisting attacks through tactics such as user authentication which addresses our goal of a log in system, and user authorization which ensures users have the rights to perform certain actions and data confidentiality which makes sure users only see the information they are allowed to see. Another strategy is the detection of attacks, this is done by requesting re-authentication if log in fails as well as the auditing of sensitive events such as making user removals and the like, and of course the logging of all actions so inconsistencies can be traced. The final strategy being recovery from attacks, this includes such as dropping a request or action once it is found to be unsafe or to be a threat and once an attack is found, restoring the system to it's state before an unsafe action was performed.
\\
\subsubsection{References}
\begin{itemize}
\item Bachmann, F. Bass, L. Klein, M. 2003. \textit{Deriving Architectural Tactics: A Step Toward Methodical Architectural Design.}  \url{http://www.sei.cmu.edu/reports/03tr004.pdf} [Accessed 8 March 2016]
\item Kazman, R. Scott, J. 2009. \textit{Realizing and Refining Architectural Tactics: Availability.} [Online]. Available: \url{http://www.sei.cmu.edu/reports/09tr006.pdf} [Accessed 8 March 2016]
\item Petre, L. 2011. \textit{Software Architectures, Lecture 2.} [Online]. Available: \url{http://www.users.abo.fi/lpetre/SA11/SALecture2.pdf} [Accessed 8 March 2016]
\item Rehn, C. 2010. \textit{Software Architectural Tactics and Patterns for Safety and Security.} [Online]. Available: \url{http://www.christian-rehn.de/wp-content/uploads/downloads/2010/06/seminar_safe_sec.pdf} [Accessed 8 March 2016]
\item Solms, F. 2016. \textit{Architectural Tactics.} [Online]. Available: \url{http://www.cs.up.ac.za/courses/COS301} [Accessed 10 March 2016]
\end{itemize}

%----------------------------------------------------------------------------------------
% Done by: Jason and Sandile (edited by Gian Paolo Buffo)
%----------------------------------------------------------------------------------------
...
%----------------------------------------------------------------------------------------
% Done by: Josef Alberts and Gian Paolo Buffo
%----------------------------------------------------------------------------------------
...
\item Interface
	\begin{itemize}
	\item HTML:  The system will be mainly web based, and will need to be integrated with Andriod for mobile deployment. Thus the 	interface should be represented with HTML 5 as it multplatform and scalable with future technologies. 
	\item CSS and Bootstrap:  Styling the web based interface should be handled through CSS 3 and the Bootstrap framework.      Bootstrap allows for a responsive, mobile interface with a profissional appearance. Importantly its' current popularity allows for future upgrades and scalable interface design.
\item Java Script , AngualrJS and Node.js :  The interface needs to remain robust, intuitive and responsive. To achieve  a responsive environment AngularJS wi l be used. Node.js is designed to build scalable network applications and using these frameworks our system will remain modurlized and will allow us to implement dependency injection.
			
	\end{itemize}
\item Server
The server side processing  needs to be handled by a suitable scripting langauge however the debate between the flexible Python and the standard PHP can be extensive. Below is a solution. 
	\begin{itemize}
	
	\item PHP: Commonly installed environment,embedding in HTML, large user base.
	 \item Python : More secure then PHP, flexible with extensive add-on modules,tends to lead to much more scalable application
	\item Solution: WPHP is a WSGI->PHP gateway that "allows you to run PHP processes inside of Python, using a WSGI gateway". In summary using both PHP and Python to achieve best of both options.
  
\end{itemize}
\item Application platform
\begin{itemize}
	
	\item Java: Promotes modulability, intergrates with the other technologies, familiar with.
	\item Andriod SDK: The mobile application will be andriod exclusive.
...
Besides the frameworks mentioned above the following will also be used.
	...
	\begin{itemize}
	\item Apache Cordova: As the system will need to be mobile device deployable, the  popular mobile application development  framework PhoneGap or more specifically the open source version  Apache Cordova  will be used to wrap the web application into an andriod application without having to remake the system in anything other then HTML, JS and CSS.
\item   ASP.NET MVC: The system will follow a MVC architecture and the free, fully supported framework for building web applications  ASP.NET MVC will help facilitate this design.
\item MEAN : A JavaScript software stack that makes use of  MongoDB, Express.js, Angular.js, and Node.js to create robust, maintanable and efficient applications.
	\end{itemize}

\subsection{Database Systems}

	\begin{itemize}
	\item MongoDB: A cross-platform document-oriented databas, NoSQL database. Using JSON-like documents with dynamic schemas, it will be able to intergrate with the system easier and faster. More importantly MongoDB allows scalabilty for our database system. Furhter simplyfing the system it uses Node.js as a plaform and is incorporated in the MEAN stack.
  
	\end{itemize}
\subsection{Operating Systems}
\begin{itemize}
	\item Desktop: Windows XP service pack 3, Vista,Windows 7, Windows  8 and Windows 10. Linux ubuntu,ArchLinux, Fedora and all debian based systems. MAC OS  X. All major web browsers will be supported.
     \item Mobile device: Andriod 2.2 Froyo to Andriod 6.0 Marshmallow and all future releases.
	\end{itemize}
\subsection {Server technologies}
\begin{itemize}
	\item Express.js: As an alternative to XAMMP Express.js is the acto standard server framework for node.js wich we are already using and is part of the MEAN stack. Express will be set up as our server to host the system.
	\end{itemize}

\subsection {Other technologies}
\begin{itemize}
	\item GitHub: Will be used as our version control system and be used to host our system source code. Thus concurrent work can take place, maintencne does not have to disrupt the system and msitakes can be undone.
	\end{itemize}
